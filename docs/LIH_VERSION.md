...
| 条件 | 动作 |
|------|------|
| 持有腿 ask **≥ 0.90** | **Hold**，等自然结算（认为大概率赢） |
| 持有腿 ask **< 0.89** | 进入软顶分批配平 |
| 曾 ≥0.90 但 **价格跌回 <0.89** | 恢复配平（不继续裸赌） |

> 与 leg1 趋势过滤共用 Binance 方向；末段 hold 以 **Polymarket ask 阈值** 为主触发。

### 第二步：软顶分批配平

- **1.15 为软顶**，不是目标价；能 ≤0.95 最好，否则尽量少亏。
- 价差已很大（如 0.40 + 0.85）→ 不追求盈利，只减损。
- **每步 5 或 10 shares**（平台最少约 5 份）：gap 小 → 5 份/步；gap 大 → 10 份/步。
- 每步下单前重算成交后合价 / port_avg；在软顶内才发单。
- 便宜腿 dip 仅在与补贵腿 **成对**、且合价仍 OK 时使用。
